...
function compilePug() {
  ...
  if (!buildLibrary) {
    fileList.push(`!${dir.src}pages/blocks-demo.pug`);
  }
  return src(fileList)
    .pipe(plumber({
...
function imagesOptiAndCopy() {
  ...
  images.push(`${dir.src}img/**/*.{png,jpg,gif,svg}`);
  return src(images)
    .pipe(debug({title: 'Found images '}))
    .pipe(imagemin([
...
exports.imagesOptiAndCopy = imagesOptiAndCopy;

// NOTE: при необходимости добавить отслеживание изменений jpg в browserSync с таском jpgToWebp
// NOTE: также при необходимости добавить jpgToWebp в таски build и default
function jpgToWebp() {
  let images = [];
  nth.blocksFromHtml.forEach(function(block) {
    let src = `${dir.blocks}${block}/img`;
...
function serve() {
  ...
  watch([`${dir.blocks}**/img/*.{jpg,jpeg,png,gif,svg}`, `${dir.src}/img/*.{jpg,jpeg,png,gif,svg}`], { events: ['all'], delay: 100 }, series(
    imagesOptiAndCopy,
    reload
  ));
...
exports.build = series(
  ...
  parallel(imagesOptiAndCopy, writeSassImportsFile, writeJsRequiresFile),
  parallel(compileSass, buildJs)
);
